...
@property (nonatomic , strong) VolumeView * customVoluView;
...
@synthesize gestureView, topBgView , titleLabel , backButton , musicPlayer , customVoluView;
...
- (void)_initGestureView
{
    ...
    // 音量控件
    customVoluView = [[VolumeView alloc] init];
    customVoluView.frame = CGRectMake(0, 0, 150, 150);
    customVoluView.center = CGPointMake(HEIGHT / 2, WIDTH / 2 - 40);
...
- (void)_initTopView
{
    ...
    horizontalConstraints = [NSLayoutConstraint
                             constraintsWithVisualFormat:@"H:|-5-[B(40)]-(-40)-[T]-5-|"
                             options:0
                             metrics:nil
                             views:@{@"B" : backButton,
                                     @"T" : titleLabel}];
...
- (void)_initBottomView
{
    ...
    horizontalConstraints = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|[P(40)][S(10)][C]-15-[I]-15-[R][F(40)][V(40)]|" options:0 metrics:nil
                                                                      views:@{@"P" : playButton,
                                                                              @"S" : spacerView,
                                                                              @"C" : currentTimeLabel,
                                                                              @"I" : progressIndicator,
...
- (void)panAction:(UIPanGestureRecognizer *)pan
{
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        NSUserDefaults * userDefaults = [NSUserDefaults standardUserDefaults];
         _systemVolume = [[userDefaults objectForKey:@"volume"] floatValue];
...
-(void)volumeChanged:(NSNotification *)noti
{
    ...
    NSUserDefaults * userDefaults = [NSUserDefaults standardUserDefaults];
    [userDefaults setObject:@(volume) forKey:@"volume"];
