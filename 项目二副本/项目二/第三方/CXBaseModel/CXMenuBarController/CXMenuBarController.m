...
- (void)setViewControllers:(NSArray<__kindof UIViewController *> *)viewControllers
{
    if (_viewControllers != viewControllers) {
        ...
        for (int i = 0; i < _viewControllers.count; i++) {
            ...
            // 06 设置将要显示视图控制器根视图的大小和位置
            viewController.view.frame = CGRectMake(_scrollView.frame.size.width * i, 0, _scrollView.frame.size.width, _scrollView.frame.size.height);
...
- (void)setSelectedIndex:(NSInteger)selectedIndex
{
    if (_selectedIndex != selectedIndex ) {
        ...
        // 把当前控制器的根视图从滑动视图上移除
        [disappearViewController viewWillDisappear:YES];
        [disappearViewController viewDidDisappear:YES];
        // 设置当前视图选中位置
